...
func (r *mongoRepository) CreateRideFareTTLIndex() error {
	index := mongo.IndexModel{
		Keys:    bson.D{{Key: "createdAt", Value: 1}},
		Options: options.Index().SetExpireAfterSeconds(1800),
	}

	_, err := r.db.Collection(db.RideFaresCollection).Indexes().CreateOne(context.Background(), index)
	if err != nil {
		return err
	} else {
		log.Printf("Successfully added TTL index to RideFare Collection.")
		return nil
	}
}
